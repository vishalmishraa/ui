...
class CSRStatusManager {
  ...
  constructor(clusterName: string, onUpdate: (status: CSRStatus) => void, onError: (message: string) => void) {
  ...
  startPolling() {
    ...
    const fetchStatus = async () => {
      try {
        const response = await fetch(`/api/cluster-status?name=${encodeURIComponent(this.clusterName)}`);
